...
<div class="user form-container">
    <form method="post" @submit.prevent="onSubmit">
        <div class="input-group">
            <input type="text" placeholder="请输手机号码" v-model.trim="iphone" required>
            <i class="icon user-icon"></i>
        </div>
        <div class="input-group">
            <input type="text" placeholder="请输入邀请码" v-model.trim="mCode" required>
    ...
    methods: {
        onSubmit() {
            if(!/^[1][3,4,5,7,8][0-9]{9}$/.test(this.iphone)){
                ...
                this.$post('/api/WxWeb/UserLogin',{'User_Role_Type':'4','User_Name': this.mCode,'User_Cell': this.iphone}).then( res =>{
                    console.log(res);
