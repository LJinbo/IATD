<template>
    <div class="editReceiver container">
        <form method="post" @submit.prevent="onSubmit">
            <div class="input-group">
                <label>姓名</label>
                <input type="text" v-model.trim="receiver.User_Name">
            </div>
            <div class="input-group">
                <label>手机号码</label>
                <input type="text" v-model.trim="receiver.User_Phone">
            </div>
            <div class="input-group">
                <label>邮政编码</label>
                <input type="text" v-model.trim="receiver.User_Code">
            </div>
            <div class="input-group">
                <label>所在区域</label>
                <select v-model="receiver.User_Province" class="selectItem" @change="selectChange('province')">
                    <option value="" selected>请选择省</option>
                    <option :value="item.Pcode" v-for="item in provinceList">{{item.ProvinceName}}</option>
                </select>
                <select v-model="receiver.User_City" class="selectItem" @change="selectChange('city')">
                    <option value="" selected>请选择市</option>
                    <option :value="item.Ccode" v-for="item in cityList">{{item.Cname}}</option>
                </select>
                <select v-model="receiver.User_Area" class="selectItem">
                    <option value="" selected>请选择区/县</option>
                    <option :value="item.Acode" v-for="item in areaList">{{item.Aname}}</option>
                </select>
            </div>
            <div class="input-group">
                <label>详细地址</label>
                <input type="text" v-model.trim="receiver.User_Street">
            </div>
            <div class="checkbox-group">
                <input type="checkbox" v-model="receiver.User_IsDeafult">
                设为默认地址
            </div>
            <div class="btn-group save">
                <button type="submit" class="btn">保存</button>
            </div>
            <div class="btn-group">
                <div class="btn" @click="deleteReceiver">删除</div>
            </div>
        </form>
    </div>
</template>
<script>
export default{
    data () {
        return {
            userInfo: {},
            receiver: {},
            addres: {}
        }
    },
    created () {
        this.userInfo = JSON.parse(localStorage.getItem('userInfo'));
        this.receiver = JSON.parse(localStorage.getItem('editReceiver'));
        this.$fetch('/api/WxWeb/GetProvinceList').then( res => {
            this.addres = res;
        })
    },
    computed: {
        provinceList () {
            this.User_City = '';
            return this.addres.province
        },
        cityList () {
            let city = this.addres.city;
            if(city){
                let arr = [];
                for(let i =0;i<city.length;i++){
                    if(city[i].PCode == this.receiver.User_Province){
                        arr.push(city[i]);
                    }
                }
                return arr;
            }
        },
        areaList () {
            let area = this.addres.area;
            if(area){
                let arr =[]
                for(let i =0;i<area.length;i++){
                    if(area[i].Ccode == this.receiver.User_City){
                        arr.push(area[i]);
                    }
                }
                return arr;
            }
        }
    },
    methods: {
        selectChange (name) {
            if( name == 'province'){
                this.receiver.User_City = '';
            }
            this.receiver.User_Area = '';
        },
        onSubmit () {
            this.$post('/api/WxWeb/SaveAddress',{'ID':this.receiver.ID,'UserID':this.userInfo.User_Id,'User_Province': this.receiver.User_Province,'User_City': this.receiver.User_City,'User_Area': this.receiver.User_Area,'User_Street': this.receiver.User_Street,'User_Code': this.receiver.User_Code,'User_Phone':this.receiver.User_Phone,'User_Name': this.receiver.User_Name,'User_IsDeafult': Number(this.receiver.User_IsDeafult)}).then(res=> {
                this.$Toast({
                    message: res.msg,
                    position: 'middle',
                    duration: 2000
                });
                let s = setTimeout( () => {
                    clearTimeout(s)
                    this.$router.push({name: 'receiver'});
                },2000)
            })
        },
        deleteReceiver () {
            this.$post('/api/WxWeb/SaveAddress',{'ID':this.receiver.ID,'UserID':this.userInfo.User_Id,'IsDelete': 1}).then(res=> {
                this.$Toast({
                    message: res.msg,
                    position: 'middle',
                    duration: 2000
                });
                let s = setTimeout( () => {
                    clearTimeout(s)
                    this.$router.push({name: 'receiver'});
                },2000)
            })
        }
    }
}
</script>
<style lang="scss">
</style>